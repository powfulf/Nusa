...
- **A fake is a claim about how the real system behaves, and nothing tests that claim unless somebody tests it.** This is not the usual failure of a fake being wrong; the fake is *better behaved* than what it stands for, so the layer above it is proved against a world that does not exist. Thirteen guards over the HTTP idempotency layer all worked correctly and all ran against an in-memory store that returns exactly what it was handed — which is precisely what `jsonb` does not do: it reordered keys, dropped whitespace and discarded a duplicate key, so the "byte-for-byte replay" those guards sat on was false at the only layer that decides it.

  It has appeared twice here, in two shapes that look nothing alike until they are stated together.

  **More obedient than the real thing.** The fake was *better behaved* than what it stood for, so the layer above was proved against a world that does not exist.

  **A different contract from the real thing.** `ReplaceBackupCodes` takes hashes and `ConsumeBackupCode` takes a plaintext code; the fake kept whatever it was handed and compared it as plaintext. A Phase 1 test seeded plaintext, submitted plaintext and passed — against a contract the real store has never had. It stayed green for a whole milestone, because **a wrong fake never announces itself**: it is consistent with itself, and every test written against it agrees.

  So: **wherever a fake stands in for an external system, write down which properties of that system the code above it relies on, and test those properties against the real thing.** Not a test that *uses* the fake — a test that *verifies* the fake. "It returns what it was given" and "it accepts a code in the form the caller sends" are both assumptions until proved on the real implementation, and the input that proves one has to be chosen to break the promise: an awkward document rather than a realistic one, a hash where a plaintext is expected, because a well-behaved value passes under either implementation and distinguishes nothing.

  The cheapest form of that test is to make the fake call the same function the real one does — the backup-code fake now matches through `auth.MatchBackupCode`, so the two cannot disagree about what counts as the same code.
